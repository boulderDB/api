...
class MigrateSettersCommand extends Command
{
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        foreach ($this->locationRepository->findAll() as $location) {
            ...
            $statement = "SELECT users.id from users INNER JOIN boulder_setters ON users.id = boulder_setters.user_id INNER JOIN boulder ON boulder_setters.boulder_id = boulder.id WHERE boulder.tenant_id = {$location->getId()} GROUP by users.id";
            $query = $connection->prepare($statement);
            $query->execute();

            $setterIds = $query->fetchAll();

            foreach ($setterIds as $setterId) {

                /**
                 * @var User $user
                 */
                $user = $this->userRepository->find($setterId);

                /**
                 * @var Setter $existingSetter
                 */
                $existingSetter = $this->setterRepository->findOneBy(["username" => $user->getUsername()]);
        ...
        foreach ($boulderSetters as $boulderSetter) {
            ...
            if (!$setter) {
                $io->warning("Setter not found " . $boulderSetter["user_id"]);
                continue;
            }
